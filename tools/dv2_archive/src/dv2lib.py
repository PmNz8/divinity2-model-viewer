#!/usr/bin/env python3
"""Unified DV2 archive backend for Divinity II: Developer's Cut.

One implementation used by the CLI, the automated tests and the GUI.  The GUI
has no archive-writing code of its own; it only calls this module.

Derivation
----------
The binary layout knowledge (header layout, path table, entry records, raw
sentinel, alignment rules, safety checks) is derived from
``context/reference/divinity2-map-tools/dv2tool.py`` (read-only reference).
This module extends that parsing core with an editing session layer:

* staged add / replace / remove operations with upfront conflict rejection;
* canonical container rebuild with byte-for-byte reuse of unchanged stored
  payloads;
* verbatim-copy "no-op" Save As that is bit-perfect by construction;
* diagnostic forced rebuild that reconstructs the container without logical
  changes;
* atomic publication: temporary file -> flush -> fsync -> full re-read and
  deep verification -> rename into place; a failure never leaves a partially
  written output file.

Layout-mode-0 acceptance: the reader accepts BOTH per-entry-aligned archives
AND the runtime-accepted experimental variant with sequential, unaligned
relative offsets (see ``mode0_unaligned_paths``). Unalignment is reported as
a variant property and is never a rejection reason; all other structural
checks (bounds, overlaps, duplicates, path safety, stream integrity) are
unconditional. Accepting this one variant confirms no other layout semantics.

Evidence discipline notes (see project TASK.md):
* The zero ``unpacked_size`` field being the marker of a *raw* payload is an
  observation encoded by the reference parser; this tool preserves it but does
  not claim runtime semantics beyond structural round-trips.
* Passing this backend's own parser and deep verify is NOT proof that a
  modified archive will be accepted by the game at runtime.

Storage modes for newly added entries must be given explicitly as ``raw`` or
``zlib``; the mode is never guessed from file extensions.
"""
